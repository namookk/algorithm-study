const data = [89.23, 82.03, 71.56, 78.82, 85.05, 84.44, 67.53
    , 71.7, 77.97, 73.77, 84.25, 67.01, 73.78, 64.19, 89.89
...
class Score{
    ...
    getStandardDeviation() { //표준편차 구하기
        const mean = this.getMean();
        let sum = 0;
        this._data.forEach(i => {
            sum += (i - mean) ** 2;
        })
        return Math.sqrt(sum / (this._data.length - 1));
    }
    getZ(num) { // Z값 구하기
        const mean = this.getMean();
        const standardDeviation = this.getStandardDeviation();
        const orgZ = ( num - mean ) / standardDeviation;
        return Math.round((orgZ + Number.EPSILON) * 100) / 100;
    }
    getStandardNormal(num) {
        const Z = this.getZ(num);
        const row = Math.floor(Math.abs(Z) * 10);
        const col = Math.floor(Math.abs(Z) * 100 ) - (row * 10);
        return SND[row][col];
    }
    getRatio(num){
        return Math.round(((1 - this.getStandardNormal(num)) + Number.EPSILON) * 10000) / 10000;
    }
    sort() {
...
class SubjectScore {
    ...
    addScore(subject, score) {
        const sc = this._subject[subject] ? this._subject[subject] : new Score();
        sc.addScore(score);
        this._subject[subject] = sc;
    }
    sort() {
        for(const key in this._subject){
            const score = this._subject[key];
            score.sort();
        }
    }
    getStandardDeviation() {
        for(const key in this._subject){
            const score = this._subject[key];
            console.log(`${key} 과목의 표준편차는 ${score.getStandardDeviation()} 입니다.`);
        }
    }
}

//console.log(run1(data));
function run1(data) { // 1번
    const score = new Score();
    score.setData(data);
    return (score.getRatio(80) - score.getRatio(70))* 100 + "%";
}


//2번
console.log("과목 점수 순으로 입력해주세요. 입력이 끝나면 Ctrl + C를 눌러 종료해주세요.");
...
rl.on('line', (input) => {
    ...
    if(inputArr.length != 2){
        console.log("입력이 잘못되었습니다. 과목 점수 순으로 입력해주세요");
    }else{
        subjectScore.addScore(inputArr[0], Number(inputArr[1]));
    }
}).on('close', function(){
    subjectScore.getStandardDeviation();
})
